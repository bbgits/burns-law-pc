...
const FormMain = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.firstName && formData.email && formData.message) {
      ...
      collection.error("UNCAUGHT ERROR");
      setSubmitStatus('error');
    }
  };
  ...
  return (
    <section className="w-full flex flex-col p-5 xs:p-10 sm:p-12 md:p-16 lg:p-20 border-b-2 border-solid border-dark dark:border-light
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded sm:px-8 md:px-12 lg:px-18 pt-6 pb-8 mt-8 mb-4 sm:mx-6 md:mx-20 lg:mx-24">
        <div className="mb-4 flex flex-wrap">
          <div className="w-full md:w-1/2 md:pr-3 mb-4 md:mb-0">
            <label className="block text-gray-700 text-md font-bold mb-2" htmlFor="firstName">Your Name</label>
            <input
              type="text"
              name="firstName"
        ...
        <div className="mb-6">
          <label className="block text-gray-700 text-md font-bold mb-2" htmlFor="message">Describe your legal problem:</label>
          <textarea
            name="message"
        ...
        <div className="flex items-center justify-end">
          <button type="submit" className="bg-black hover:bg-gray-900 text-lg text-white font-bold py-2 px-16 rounded focus:outline-none focus:shadow-outline">Submit</button>
        </div>
      </form>
